...
/**
 * Admin Filter - Checks if user has admin role Protects admin-only pages
 */
...
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Lấy action và chuyển về chữ thường
        ...
        if (isAdminAction(action)) {
            ...
            if (session != null) {
                ...
                if (user != null && user.isAdmin()) {
                    ...
                    // Chuyển hướng về trang danh sách với cờ báo lỗi
                    String redirectUrl = httpRequest.getContextPath() + "/student?action=list&auth_denied=true";
                    httpResponse.sendRedirect(redirectUrl);
                }
            } else {
                // No session (hoặc session đã hết hạn), chuyển hướng đến login
